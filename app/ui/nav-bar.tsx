  ...
  return (<nav className="bg-sky-500">
    <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
      <div className="relative flex h-12 items-center justify-between">
        ...
        <div ref={accountMenuRef} className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
    ...
    {showNavMenu && (
      <div ref={navMenuRef} className="space-y-1 p-2">
        {NAV_ITEMS.map((item) => (
          <Link onClick={() => setShowNavMenu(false)} href={item.href} key={item.name} className={`text-white hover:bg-gray-700 hover:text-white block rounded-md px-3 py-2 text-base font-medium ${pathname === item.href ? 'bg-sky-700' : ''}`}> {item.name}</Link>
        ))}
      </div>
    )}
  </nav >
